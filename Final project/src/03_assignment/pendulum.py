...
class Pendulum:
    '''
    Define a class Pendulum with nj joints.
    ...
    * visuals: the list of all the 'visual' 3D objects to render the robot, each element of the list being
    ...
    def __init__(self, nj=1, noise_stddev=0.0, nu=11, umax=2.0):
        '''Create a Pinocchio model of a N-pendulum, with N the argument <nj>.'''
        self.viewer     = Display()
        self.visuals    = []
        self.model      = pin.Model()
        self.createPendulum(nj)
        ...
        self.nu = nu                    # Number of discretizations for joint torque
        self.DU = 2*self.umax/self.nu   # Discretization resolution for joint torque

    def createPendulum(self, nj, rootId=0, prefix='', jointPlacement=None):
...
#    def nobs(self): return self.nx+self.withSinCos
    ...
    def d2cu(self, iu):
        if isinstance(iu, np.int64):
            iu = np.clip(iu, 0, self.nu - 1) - (self.nu - 1)/2
        else:
            iu = np.array([np.clip(i, 0, self.nu - 1) - (self.nu - 1)/2 for i in iu])
        return iu*self.DU

    def reset(self, x0=None):
        ''' Reset the state of the environment to x0 '''
        if x0 is None: 
            q0 = np.pi*(np.random.rand(self.nq, 1)*2 - 1)
            v0 = np.random.rand(self.nv, 1)*2 - 1
            x0 = np.vstack([q0, v0])
    ...
    def dynamics(self, x, u, display=False):
        ...
        v = x[self.nq:]
        u = np.clip(np.reshape(np.array(u),self.nv),-self.umax,self.umax)
        ...
        for i in range(self.NDT):
            ...
            M = self.data.M
            b = self.data.nle
            a = inv(M)@(u - np.reshape(self.Kf*v, self.nv) - b)
            a = a.reshape(self.nv, 1) + np.random.randn(self.nv, 1)*self.noise_stddev
            self.a = a

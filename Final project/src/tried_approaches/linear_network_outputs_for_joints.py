...
def get_critic(nx, nu, nj):
    ...
    state_out4 = layers.Dense(32, activation="selu")(state_out3)
...
def update(x_batch, u_batch, reward_batch, x_next_batch):
    ...
    with tf.GradientTape() as tape:
        ...
        target_values = tf.math.reduce_min(tf.reshape(Q_target(x_next_batch, training=False), [len(x_next_batch), env.nj, env.nu]), axis=2, keepdims=True)
        target_values = tf.math.reduce_sum(target_values, axis=1)  # sum up q values of the best action for each joint
        ...
        for i in range(len(u_batch)):
            for j in range(len(u_batch[i])):
                u_batch_indexed.append([j, i, int(u_batch[i][j])])
        u_batch_indexed = tf.stack(u_batch_indexed)
        # Compute batch of Values associated to the sampled batch of states
        x_batch = tf.transpose(tf.squeeze(x_batch, axis=0), perm=[1, 0, 2])
        Q_values = tf.reshape(tf.expand_dims(tf.gather_nd(tf.reshape(Q(x_batch, training=True), [len(x_next_batch), env.nj, env.nu]), u_batch_indexed), 1), [env.nj, len(x_next_batch)])
        Q_values = tf.math.reduce_sum(Q_values, axis=0)  # sum up q values of the chosen action for each joint
...
class Agent:
    ...
    def step(self, net, epsilon=0.0):
        ...
        # Otherwise select action based on qvalues
            ...
            u = np.argmin(tf2np(q_values).reshape((self.env.nj, self.env.nu)), axis=1)
...
def train(net, target_net, agent):
    ...
    try:
        for k in range(NEPISODES):
            ...
            for _ in range(MAX_EPISODE_LENGTH):
                ...
                frame_idx +=1
                # Computes the current epsilon with linear decreasing
    ...
    return h_ctg, net.get_weights()  # best_weights_so_far
...
def render_greedy_policy(net, env, x0=None):
    ...
    for _ in range(MAX_EPISODE_LENGTH):
        ...
        # Get qvalues and select the index of the minimum for each joint
        q_values = net(state_tensor)
        u = np.argmin(tf2np(q_values).reshape((env.nj, env.nu)), axis=1)
...
def evalutate_greedy_policy(net, env, n_step=8):
    ...
    for _ in range(MAX_EPISODE_LENGTH):
        ...
        # Get qvalues and select the index of the minimum for each joint
        q_values = net(state_tensor)
        u = np.argmin(tf2np(q_values).reshape((len(x), env.nj, env.nu)), axis=2)
...
if __name__=='__main__':
    ...
    umax = 4.0  # maximum torque for the joints of the pendolum

    env = Pendulum(nbJoint=nj, nu=nu, umax=umax)
    ...
    # Q.load_weights("qbestweightsx2.h5")
    ...
    Q.save_weights("qbestweightsx2.h5")
